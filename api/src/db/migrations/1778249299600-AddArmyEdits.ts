...
export class AddArmyEdits1778249299600 implements MigrationInterface {
    name = 'AddArmyEdits1778249299600'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`troop_types\` (\`id\` varchar(36) NOT NULL, \`key\` varchar(255) NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` text NULL, \`category\` enum ('INFANTRY', 'RANGED', 'CAVALRY', 'SPECIAL', 'PEASANT') NOT NULL, \`cost_per_100\` int NOT NULL DEFAULT '0', \`attack\` float NOT NULL DEFAULT '1', \`defense\` float NOT NULL DEFAULT '1', \`upkeep_per_100\` int NOT NULL DEFAULT '100', \`tech_requirement\` varchar(255) NULL, \`building_requirement\` varchar(255) NULL, UNIQUE INDEX \`IDX_4b3eadb1a042c2d26898605da9\` (\`key\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`piety\` int NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`piety\``);
